...
// etsitään web-sivulta kohta, johon js-koodin tulos lisätään (id='pictures')
const targetElem = document.querySelector('#pictures');

// käydään yllä olevan taulukon tiedot läpi for...of -toistolla
for (let kuva of picArray)  {
  // luodaan uusi article-elementti, jonka sisään yhden kuvan tiedot tulevat
  let articleElem = document.createElement('article');

  // ** määritetään articlen sisään tulevat elementit **

  // määritellään articlen otsikko
  let h2Elem = document.createElement('h2');
  // lisätään otsikon teksti kuvan title-avaimen avulla
  h2Elem.innerHTML = kuva.title;

  // määritellään articlen figure tiedot
  let figureElem = document.createElement('figure');
  // figuren sisään tuleva img-elementti
  let imgElem = document.createElement('img');
  imgElem.src = kuva.image.medium;
  imgElem.alt = kuva.title;
  // figuren sisään tuleva figcaption-elementti
  let figcaptionElem = document.createElement('figcaption');
  figcaptionElem.innerHTML = kuva.caption;
  // lisätään figure-elementin sisään img- ja figcaption-elementit
  figureElem.appendChild(imgElem);
  figureElem.appendChild(figcaptionElem);

  // määritellään articlen description-tiedot
  let pElem = document.createElement('p');
  pElem.innerHTML = kuva.description;

  // ** kootaan palapeli' eli lisätään articlen sisään sinne tulevat muut elementit **
